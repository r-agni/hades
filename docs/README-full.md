...
The plan in `<repo root>\plan.md` defines the Track 2-4 target:
...
The RL implementation source in `<rl-training branch>` provides the training-oriented branch:
...
- `HADES_README.md` exists in `<repo root>`.
